...
public class ExpensesController implements Initializable {
    ...
    @FXML private final ObservableList<String> categoryList = FXCollections
            .observableArrayList("Choose a category for your item", "Groceries", "Rent", "Utilities",
                    "Subscriptions", "Merchandise","Restaurants", "Transportation", "Other");
    ...
    public void addItemClick()
    {
        ...
        try {
            ...
            if (str_cost.matches("\\${0,1}[0-9]+\\.{0,1}([0-9]{0,2})")
                ...
                // if the added expense brings the total Expenses over the budget limit, throw error message
                if ((Double.parseDouble(str_cost) + Expenses.getTotalAllExpenses())
                        > SetBudgetController.budget.getBudgetLimit()){

                    Alert invalid_alert = new Alert(Alert.AlertType.ERROR);
                    invalid_alert.setTitle("Over the Limit!");
                    invalid_alert.setContentText("The current expense total will go over your budget limit. ("
                            + SetBudgetController.budget.getBudgetLimit() + ")"
                            + "Please add a lesser amount or increase your budget limit.");
    ...
    public void changeScreenToHome(ActionEvent event) throws IOException
    {
        ...
        if(dataList != null && Expenses.getExpensesTable() != null) {
            ...
            Expenses.setSubscriptionExpense();
